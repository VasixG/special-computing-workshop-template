...
/**
 * The {@code TelegramApiConnector} class is a connector
 * to the Telegram API using the access token and user ID obtained from environment variables.
 *
 ...
 * TELEGRAM_TOKEN is the access token for the Telegram bot, and USER_ID is the identifier
 * of the user. </p>
 */
        ...
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            ...
            try (var response = client.execute(httpPost)) {
                ...
                if (jsonElement.isJsonObject() && jsonElement.getAsJsonObject().has("error_code")) {
                    System.err.println("Error from Telegram API: " + jsonElement.getAsJsonObject().get("description").getAsString());
                }
            }
        } catch (IOException e) {
